...
function MarkdownCodeBlock({
  ...
  return (
    <div className="overflow-hidden rounded-sm border">
      <div className="bg-background/70 px-1 py-0.5 text-sm text-foreground">
        {language}
      </div>
      <CodeBlock code={src} language={language}>
        <CodeBlock.Code
          className={cn(
            "bg-foreground px-2 py-1.5 whitespace-pre-wrap",
            className
          )}
        >
...
export const Markdown = ({
  ...
  const normalizedContent = useMemo(
    () => normalizeMarkdownForRendering(children),
    [children]
  )

  const headerRange = useMemo(
    () => getMarkdownHeaderRange(children),
    [children]
  )
  ...
  const initialComponents = useMemo((): Partial<Components> => {
    ...
    const offset =
      normalizedHeaderLevel && headerRange.min_header_level
    ...
    const headerRenderers =
      parentHeaderRenderers ??
      ([1, 2, 3, 4, 5, 6] as const).reduce<Partial<Components>>(
        (acc, level) => {
    ...
    return {
      ...headerRenderers,
      hr: () => <div className="my-4 h-px w-full bg-foreground/20" />,
      p: ({ children }) => (
        <div className={cn("text-base", DefaultPartStyle)}>{children}</div>
      ),
      ol: ({ children }) => (
        <ol className="my-2 ml-8 list-decimal text-base">{children}</ol>
      ),
      ul: ({ children }) => (
        <ul className="my-2 ml-8 list-disc text-base">{children}</ul>
      ),
      defs: ({ children }) => (
        <div className={cn("text-base", DefaultPartStyle)}>{children}</div>
      ),
      ...
      strong: ({ children }) => (
        <strong className={cn("text-base font-semibold", DefaultPartStyle)}>
          {children}
        </strong>
      ),
      table: ({ children }) => (
        <div className="overflow-hidden rounded-sm border">
          <table
            width="100%"
            className="rounded-sm bg-background/70 text-foreground/90"
          >
            {children}
          </table>
        </div>
      ...
      th: ({ children }) => (
        <th className="border-r px-2 py-1.5 last:border-r-0">{children}</th>
      ),
      td: ({ children }) => (
        <td className="border-r px-2 py-1.5 last:border-r-0">{children}</td>
      ),
      code: ({ children, className }) => {
        if (typeof children !== "string") {
          return (
            <span className={cn("text-base", DefaultPartStyle)}>
              {children}
            </span>
          )
        }
          ...
          return (
            <div
              dangerouslySetInnerHTML={{
                __html: children.replace(/&lt;/g, "<").replace(/&gt;/g, ">"),
              }}
            />
          )
        }
        ...
        if (language && textLanguages.has(language)) {
          return (
            <div
              className={cn(
                "rounded-sm border bg-background px-2 py-1.5 text-base text-blue-700 dark:text-blue-400",
                className
              )}
            >
              {children}
        ...
        if (!language || markdownLanguages.has(language)) {
          return (
            <Markdown
              ...
              className={cn(
                "language-markdown w-full overflow-hidden text-base whitespace-normal",
                DefaultPartStyle
              )}
            >
              {codeContent}
    ...
    currentMinHeaderLevel,
    fontLevel,
    headerRange,
    normalizedHeaderLevel,
    parentHeaderRenderers,
  ])

  const fontSizeConfig = {
    "[--text-xs:calc(0.75rem*0.8)] [--text-sm:calc(0.875rem*0.8)] [--text-base:calc(1rem*0.8)] [--text-lg:calc(1.125rem*0.8)] [--text-xl:calc(1.25rem*0.8)] [--text-2xl:calc(1.5rem*0.8)] [--text-3xl:calc(1.875rem*0.8)] [--text-4xl:calc(2.25rem*0.8)] [--text-5xl:calc(3rem*0.8)]":
      fontLevel === "small",
    "[--text-xs:calc(0.75rem*1.25)] [--text-sm:calc(0.875rem*1.25)] [--text-base:calc(1rem*1.25)] [--text-lg:calc(1.125rem*1.25)] [--text-xl:calc(1.25rem*1.25)] [--text-2xl:calc(1.5rem*1.25)] [--text-3xl:calc(1.875rem*1.25)] [--text-4xl:calc(2.25rem*1.25)] [--text-5xl:calc(3rem*1.25)]":
      fontLevel === "large",
  }
  ...
  return (
    <div className={cn("overflow-hidden text-base", className, fontSizeConfig)}>
      <ReactMarkdown
        key={blockId}
