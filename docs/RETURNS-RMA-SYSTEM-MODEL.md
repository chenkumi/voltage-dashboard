...
- 外部 Agent：查詢安全資料、套用政策、整理證據、填寫草稿與產生客服內容，不能
  執行最終操作。
- 外部通路：第一階段只以本機種子資料表示已匯入的退貨申請。

## 5. 完整 RMA 流程

1. 外部通路匯入退貨申請，或營運人員從 Order Detail 建立退貨申請。
2. 選擇退貨品項、數量、原因及顧客陳述。
3. 系統依固定政策產生資格判斷。
4. 使用者核准或拒絕退貨資格。
5. 核准後進入等待寄回，並追蹤寄回期限。
6. 倉庫登記收貨。
7. 倉庫逐商品完成結構化驗貨。
8. 系統依驗貨通過數量與原始實付金額產生退款計算。
9. 使用者將退款計算送交 Refund Approvals。
10. 核准人核准、拒絕或退回修改。
11. 核准後進入待退款，不直接標記退款成功。
12. 財務或營運人員在外部支付系統操作後，記錄成功或失敗。
13. 退款成功後完成 RMA，並保留完整 Timeline。
...
- `apply_return_review_draft`：填寫安全證據碼、營運摘要、下一步與客服回覆草稿。
- `get_return_review_state`：驗證 draft、policy 與資料版本。
...
筆數，拒絕額外欄位，驗證所有實體與版本，並在 route 改變後重新 discovery。成功草稿
mutation 必須搭配 verifier。
...
讀取安全 RMA、執行固定政策、整理證據、填寫營運及客服草稿，並停在使用者資格決定前。

### `refund-review-preparation`

讀取驗貨結果與退款計算、檢查版本、解釋全額退款依據、導覽 Refund Approval，並停在
人工核准前。
